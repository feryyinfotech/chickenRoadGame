...
const UserPermission = () => {
  ...
  return (
    <div className="!flex justify-center items-center w-full">
      <div className="p-5 lg:w-1/2 md:w-3/4 w-full bg-white !bg-opacity-10 !rounded-lg">

        <p className="!text-center !text-white font-bold !py-4 text-lg">Change User Permission</p>
        <div className="grid grid-cols-1 gap-[6%] gap-y-4">

          <div>
            <p className="!text-white font-bold mt-5">User ID </p>
            <TextField
              className="!bg-white"
              fullWidth
              id="user_id"
            ...
            /> {fk.values.user_id ? (
              result ? (
                <div className="no-error">Referral From: {result?.full_name}<span className="!text-white"> ({result?.user_type})</span></div>
              ) : (
                <div className="error">Invalid Referral Id</div>
              )
            ) : null}
            <p className="!text-white font-bold mt-5">Select Profile Type</p>
            <TextField
              ...
              onChange={fk.handleChange}
            >
              <MenuItem className="!text-black" value={4}>Dummy User</MenuItem>
              <MenuItem className="!text-black" value={1}>User</MenuItem>
              <MenuItem className="!text-black" value={2}>Admin</MenuItem>
              <MenuItem className="!text-black" value={3}>Super Admin</MenuItem>
              <MenuItem className="!text-black" value={5}>Support Agent</MenuItem>

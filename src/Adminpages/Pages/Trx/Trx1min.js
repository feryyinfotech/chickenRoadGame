...
const TRXColorPrediction1Min = () => {
  ...
  async function manuallyWinningAPI(num_type) {
    ...
    try {
      const res = await axiosInstance.post(API_URLS.trx_result_api, newreqBody);
      toast(res?.data?.msg);
  ...
  async function manuallyAmountAPI() {
    ...
    try {
      const res = await axiosInstance.post(API_URLS.trx_amount_api, newreqBody);
      // toast(res?.data?.msg);
...
export default TRXColorPrediction1Min;

...
const K3ColorPrediction1Min = () => {
  ...
  async function manuallyWinningAPI(num_type) {
    ...
    try {
      const res = await axiosInstance.post(API_URLS.k3_result_api, newreqBody);
      toast(res?.data?.msg);
  ...
  const image_array = [ pr3, pr4, pr5, pr6, pr7, pr8, pr9,ten,eleven,twelve,thirteen,fourteen,fifteen,sixteen,seventeen,eighteen];
  ...
  async function manuallyAmountAPI() {
    const newreqBody = {
      game_id: "1",
    };
    try {
      const res = await axiosInstance.post(API_URLS.k3_amount_api, newreqBody);
      // toast(res?.data?.msg);
  ...
  return (
    <div>
        ...
        {image_array?.map((item, index) => (
          <div key={index}>
            <img
              ...
              onClick={() => handleImageClick(index + 3)} // Use SweetAlert here
            />
      ...
      {/* <div className="!mt-10 !flex-wrap flex justify-center gap-10">
        {["Big", "Small", "Odd", "Even"]?.map((item, index) => (
          <div key={index}>
...
export default K3ColorPrediction1Min;

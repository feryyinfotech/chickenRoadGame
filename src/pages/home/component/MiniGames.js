...
function MiniGames({ alljiligames, getGamnesbyID }) {

    const style = {
        winbox: { background: 'linear-gradient(175deg, rgba(1,19,65,1) 0%, rgba(35,196,185,1) 100%)', borderRadius: '20px', height: '120px', marginBottom: '0px', },
        positiongame: {
                        ...
                        return <>
                            <Grid item xs={6}>
                                <Box sx={style.winbox} onClick={() => getGamnesbyID(item?.id)}>
                                    <Box component='img' className="!rounded-2xl" src={item?.img} sx={{ width: '100%', height: '100%', borderRadius: '20px', }}></Box>
                                </Box>
                            </Grid>
                        </>
                    })}
